...
class TableController extends AppController
{
    ...
    public function seeContentTable(int $id_table){
        ...
        $this->render('content_table', ['tables' => $tables, 'user_tables' => $user_tables]); //by widziec serduszka
    }


    public function likeSelectTable(int $id_table)
    {
       $this->tableRepository->likeTable($id_table, $_SESSION['id']); //id_user
    ...
    {
        $this->tableRepository->dislikeTable($id_table, $_SESSION['id']); //id_user
        http_response_code(200);
    }
